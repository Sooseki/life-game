...
export const Grid = () => {
  class Grid {
        ...
        for (let col in this.grid[row]) {
          ...
          for (let rowIndex = -1; rowIndex <= 1; rowIndex++) {
            for (let colIndex = -1; colIndex <= 1; colIndex++) {
              ...
              if (
                this.grid[rowNum + rowIndex] &&
                this.grid[rowNum + rowIndex][colNum + colIndex] &&
                this.grid[rowNum + rowIndex][colNum + colIndex].getState() ==
                  true
              ) {
                siblings++;
              } else {
                // ADD SIBLINGS DEAD CELLS TO CHECK IF ALIVE
                this.deadCellToCheck[rowNum + rowIndex][colNum + colIndex] =
                  true;
              }
            }
          }
    ...
    resetDeadCellTable() {
      ...
      for (
        let row = this.delay - 1;
        row < this.height + (this.delay + 1);
        row++
      ) {
